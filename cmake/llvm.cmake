# Copyright 2023 The TonyDialect Authors
#
...
#   tonydialect_llvm_configure_bundled() : Configures the bundled LLVM and
#     sub-projects from third_party.
#   tonydialect_llvm_configure_installed() : Configures from installed LLVM and
#     sub-projects.
#   tonydialect_add_llvm_external_project(...) : Adds an external LLVM project in
#     a similar fashion to LLVM_EXTERNAL_PROJECTS define (but in a way that
#     does not require a source build of LLVM).

macro(tonydialect_llvm_configure_bundled)
  message(STATUS "Adding bundled LLVM source dependency")
  tonydialect_llvm_set_bundled_cmake_options()

  # Enable MLIR Python bindings if TonyDialect Python bindings enabled.
  if(TonyDialect_BUILD_PYTHON_BINDINGS)
  ...
  set(LLVM_CMAKE_DIR "${TonyDialect_BINARY_DIR}/llvm-project/lib/cmake/llvm")
  ...
  set(LLVM_INCLUDE_DIRS
    ${TonyDialect_SOURCE_DIR}/third_party/llvm-project/llvm/include
    ${TonyDialect_BINARY_DIR}/llvm-project/include
  )
  set(MLIR_INCLUDE_DIRS
    ${TonyDialect_SOURCE_DIR}/third_party/llvm-project/mlir/include
    ${TonyDialect_BINARY_DIR}/llvm-project/tools/mlir/include
  )
  set(LLD_INCLUDE_DIRS
    ${TonyDialect_SOURCE_DIR}/third_party/llvm-project/lld/include
    ${TonyDialect_BINARY_DIR}/llvm-project/tools/lld/include
  )

  set(LLVM_BINARY_DIR "${TonyDialect_BINARY_DIR}/llvm-project")
  set(LLVM_TOOLS_BINARY_DIR "${LLVM_BINARY_DIR}/bin")
  set(LLVM_EXTERNAL_LIT "${TonyDialect_SOURCE_DIR}/third_party/llvm-project/llvm/utils/lit/lit.py")
endmacro()

macro(tonydialect_llvm_configure_installed)
...
macro(tonydialect_llvm_set_bundled_cmake_options)
  # When enabling an TonyDialect CPU backend, automatically enable these targets.
  set(TonyDialect_DEFAULT_CPU_LLVM_TARGETS "X86;ARM;AArch64;RISCV"
  ...
  set(TonyDialect_CLANG_TARGET)
  set(TonyDialect_LLD_TARGET)

  # Unconditionally enable some other cheap LLVM tooling.
  set(TonyDialect_LLVM_LINK_TARGET llvm-link)

  # Unconditionally enable mlir.
  list(APPEND LLVM_ENABLE_PROJECTS mlir)

  # Configure LLVM based on enabled TonyDialect target backends.
  message(STATUS "TonyDialect compiler target backends:")
  if(TonyDialect_TARGET_BACKEND_CUDA)
    message(STATUS "  - cuda")
    list(APPEND LLVM_TARGETS_TO_BUILD NVPTX)
    set(TonyDialect_CLANG_TARGET clang)
  endif()
  if(TonyDialect_TARGET_BACKEND_LLVM_CPU)
    message(STATUS "  - llvm-cpu")
    list(APPEND LLVM_TARGETS_TO_BUILD "${TonyDialect_DEFAULT_CPU_LLVM_TARGETS}")
    set(TonyDialect_CLANG_TARGET clang)
    set(TonyDialect_LLD_TARGET lld)
  endif()
  if(TonyDialect_TARGET_BACKEND_LLVM_CPU_WASM)
    message(STATUS "  - llvm-cpu (wasm)")
    list(APPEND LLVM_TARGETS_TO_BUILD WebAssembly)
    set(TonyDialect_CLANG_TARGET clang)
    set(TonyDialect_LLD_TARGET lld)
  endif()
  if(TonyDialect_TARGET_BACKEND_METAL_SPIRV)
    message(STATUS "  - metal-spirv")
  endif()
  if(TonyDialect_TARGET_BACKEND_ROCM)
    message(STATUS "  - rocm")
    list(APPEND LLVM_TARGETS_TO_BUILD AMDGPU)
  endif()
  if(TonyDialect_TARGET_BACKEND_VULKAN_SPIRV)
    message(STATUS "  - vulkan-spirv")
  endif()
  if(TonyDialect_TARGET_BACKEND_VMVX)
    message(STATUS "  - vmvx")
  endif()
  if(TonyDialect_TARGET_BACKEND_WEBGPU)
    message(STATUS "  - webgpu")
  endif()

  if(TonyDialect_CLANG_TARGET)
    list(APPEND LLVM_ENABLE_PROJECTS clang)
  endif()
  if(TonyDialect_LLD_TARGET)
...
# tonydialect_add_llvm_external_project(name location)
...
function(tonydialect_llvm_add_external_project name location)
...
# tonydialect_llvm_add_usage_requirements(llvm_library usage_library)
# Adds |usage_library| as a link dependency of |llvm_library| in a way that
# is legal regardless of whether imported or built.
function(tonydialect_llvm_add_usage_requirements llvm_library usage_library)
